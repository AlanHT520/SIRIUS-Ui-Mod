...
public class OptionBinder {
    ...
    public static void setOptionValue(String key, double value, Options opts) {
        switch (key) {
            ...
            case "soundVolume" -> opts.getSoundSourceOptionInstance(SoundSource.MASTER).set(value);
            case "musicVolume" -> opts.getSoundSourceOptionInstance(SoundSource.MUSIC).set(value);
            case "recordVolume" -> opts.getSoundSourceOptionInstance(SoundSource.RECORDS).set(value);
            case "weatherVolume" -> opts.getSoundSourceOptionInstance(SoundSource.WEATHER).set(value);
            case "hostileVolume" -> opts.getSoundSourceOptionInstance(SoundSource.HOSTILE).set(value);
            case "ambientVolume" -> opts.getSoundSourceOptionInstance(SoundSource.AMBIENT).set(value);
            case "voiceVolume" -> opts.getSoundSourceOptionInstance(SoundSource.VOICE).set(value);
            case "blockVolume" -> opts.getSoundSourceOptionInstance(SoundSource.BLOCKS).set(value);
            case "playerVolume" -> opts.getSoundSourceOptionInstance(SoundSource.PLAYERS).set(value);
            case "neutralVolume" -> opts.getSoundSourceOptionInstance(SoundSource.NEUTRAL).set(value);
    ...
    public static void applyVolume(String key, Options opts) {
        SoundSource source = getSoundSource(key);
        if (source != null) {
            double volume = opts.getSoundSourceOptionInstance(source).get();
            Minecraft.getInstance().getSoundManager().updateSourceVolume(source, (float) volume);
        }
    }

    public static boolean isAudioOption(String key) {
        return key != null && key.endsWith("Volume");
    }

    private static SoundSource getSoundSource(String key) {
        return switch (key) {
            case "soundVolume" -> SoundSource.MASTER;
            case "musicVolume" -> SoundSource.MUSIC;
            case "recordVolume" -> SoundSource.RECORDS;
            case "weatherVolume" -> SoundSource.WEATHER;
            case "hostileVolume" -> SoundSource.HOSTILE;
            case "ambientVolume" -> SoundSource.AMBIENT;
            case "voiceVolume" -> SoundSource.VOICE;
            case "blockVolume" -> SoundSource.BLOCKS;
            case "playerVolume" -> SoundSource.PLAYERS;
            case "neutralVolume" -> SoundSource.NEUTRAL;
            default -> null;
        };
    }
