...
import net.alan.gui.data.widget.Position;
import net.alan.gui.data.widget.Size;
import net.alan.gui.data.widget.*;
...
public class WidgetFactory {
    ...
    public static Widget create(JsonObject json, ResourceManager manager, ActionExecutor executor) {
        ...
        Widget widget = switch (type) {
            ...
            case "cycle_button" -> {
                ...
                int bgColor = json.has("background_color")
                        ? (int) BackgroundRenderer.parseColor(json.get("background_color").getAsString())
                        : 0xFFAAAAAA;
                int hBgColor = json.has("highlighted_background_color")
                        ? (int) BackgroundRenderer.parseColor(json.get("highlighted_background_color").getAsString())
                        : 0xFFCCCCCC;
                yield new CycleButtonWidget(id, layout, variables, member, cycleProps, tex, textProps, children, stateKey, bgColor, hBgColor);
            }
            case "selector" -> {
                ...
                int bgColor = json.has("background_color")
                        ? (int) BackgroundRenderer.parseColor(json.get("background_color").getAsString())
                        : 0xFF3A3A3C;
                int selColor = json.has("selected_color")
                        ? (int) BackgroundRenderer.parseColor(json.get("selected_color").getAsString())
                        : 0xFF4A6FA5;
                int hlColor = json.has("highlighted_color")
                        ? (int) BackgroundRenderer.parseColor(json.get("highlighted_color").getAsString())
                        : 0xFF505052;
                yield new SelectorWidget(id, layout, variables, member, optionKey, segments, tex, textProps, children, stateKey, bgColor, selColor, hlColor);
            }
            case "arrow_switch" -> {
                String optionKey = json.has("option_key") ? json.get("option_key").getAsString() : null;
            ...
            case "dropdown" -> {
                ...
                String stateKey = json.has("state_key") ? json.get("state_key").getAsString() : null;
                yield new DropdownWidget(id, layout, variables, member, optionKey, values, buttonTex, dropdownTex, itemNormalTex, itemHighlightedTex, textProps, children, stateKey);
            }
            case "edit_box" -> {
            ...
            case "dynamic_list" -> {
                ...
                Supplier<List<DynamicListData>> source = switch (dsType) {
                    ...
                    case "game_rules" -> GameRulesDataSource::load;
                ...
                JsonObject recreateBtnTemplate = null;
                if (json.has("row_template") && json.get("row_template").isJsonObject()) {
                    ...
                    recreateBtnTemplate = resolveButtonTemplate(rowTemplateJson, "recreate_button", manager);
                }
                ...
                if (json.has("search") && json.get("search").isJsonObject()) {
                    ...
                    int sx = sch.has("x") ? sch.get("x").getAsInt() : 0;
                    int sy = sch.has("y") ? sch.get("y").getAsInt() : 0;
                    int sw = sch.has("width") ? sch.get("width").getAsInt() : 160;
                    int sh = sch.has("height") ? sch.get("height").getAsInt() : 16;
                    String hint = sch.has("hint") ? sch.get("hint").getAsString() : "";
                    String initVal = sch.has("initial_value") ? sch.get("initial_value").getAsString() : "";
                    int maxLen = sch.has("max_length") ? sch.get("max_length").getAsInt() : 32;
                    boolean bordered = !sch.has("bordered") || sch.get("bordered").getAsBoolean();
                    String textColor = sch.has("text_color") ? sch.get("text_color").getAsString() : "0xE0E0E0";
                    String hintColor = sch.has("hint_color") ? sch.get("hint_color").getAsString() : "0x808080";
                    dw.enableSearch(sx, sy, sw, sh, hint, initVal, maxLen, bordered, textColor, hintColor);
                }
                yield dw;
            }
            case "pack_list" -> {
        ...
        if (widget instanceof BaseWidget baseWidget) {
            WidgetSoundConfig soundConfig = parseSoundConfig(json);
            if (soundConfig != null) {
                baseWidget.setSoundConfig(soundConfig);
            }
        }
    ...
    private static WidgetSoundConfig parseSoundConfig(JsonObject json) {
        if (!json.has("sound")) return null;
        
        JsonElement soundElement = json.get("sound");
        if (!soundElement.isJsonObject()) return null;
        
        JsonObject soundObj = soundElement.getAsJsonObject();
        WidgetSoundConfig config = new WidgetSoundConfig();
        
        if (soundObj.has("click")) {
            config.setClick(parseSoundEvent(soundObj.get("click")));
        }
        if (soundObj.has("hover")) {
            config.setHover(parseSoundEvent(soundObj.get("hover")));
        }
        if (soundObj.has("value_change")) {
            config.setValueChange(parseSoundEvent(soundObj.get("value_change")));
        }
        if (soundObj.has("drag_start")) {
            config.setDragStart(parseSoundEvent(soundObj.get("drag_start")));
        }
        if (soundObj.has("drag_end")) {
            config.setDragEnd(parseSoundEvent(soundObj.get("drag_end")));
        }
        if (soundObj.has("focus")) {
            config.setFocus(parseSoundEvent(soundObj.get("focus")));
        }
        if (soundObj.has("release")) {
            config.setRelease(parseSoundEvent(soundObj.get("release")));
        }
        
        return config;
    }

    private static SoundEventConfig parseSoundEvent(JsonElement element) {
        SoundEventConfig config = new SoundEventConfig();
        
        if (element.isJsonPrimitive()) {
            config.setId(element.getAsString());
            config.setVolume(1.0f);
            config.setPitch(1.0f);
        } else if (element.isJsonObject()) {
            JsonObject obj = element.getAsJsonObject();
            if (obj.has("id")) {
                config.setId(obj.get("id").getAsString());
            }
            config.setVolume(obj.has("volume") ? obj.get("volume").getAsFloat() : 1.0f);
            config.setPitch(obj.has("pitch") ? obj.get("pitch").getAsFloat() : 1.0f);
            if (obj.has("throttle_ticks")) {
                config.setThrottleTicks(obj.get("throttle_ticks").getAsInt());
            }
        } else {
            return null;
        }
